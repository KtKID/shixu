...
/**
 * 「全部收藏」视图（homepage feat07）：默认不截取地展示本地收藏库全部收藏，
 * 按收藏时间（createdAt）从新到旧，未登录照常可用（feat01 场景4）。
 * 筛选/搜索/收藏时间（feat05/feat06/feat08）：有筛选时标题切「筛选结果 · N 条命中」（feat05 场景1/6）、
 * 搜索关键词与四维筛选、收藏时间取「且」叠加，无匹配显示对应空态文案。
 */
...
export default function LibrarySection({ onNavigateImport }: { onNavigateImport: () => void }) {
  ...
  /** 命中集合（applyFilter 已按收藏时间新到旧排列），默认即全量（feat07）。 */
  ...
  const visible = useMemo(() => {
    ...
    return applyFilter(state.bookmarks, filter, syncCtx);
  }, [state, filter, syncCtx]);
      ...
      <header className="page-head">
        <div>
          <h2 className="page-title serif">
            {filtering ? `筛选结果 · ${visible.length} 条命中` : '全部收藏'}
          </h2>
          <p className="page-tagline">把收藏变成可再次遇见的线索。</p>
          ...
          <div className="cards">
            {visible.length === 0 ? (
              <div className="empty">
                {searching
                  ? '没有找到相关收藏'
                  : filter.pendingOnly && !hasDimensionFilter(filter) && filter.timeRange === 'all'
                    ? '没有待同步的收藏，一切都已在服务器上'
