...
export default function RecentSection({
  ...
  useEffect(() => {
    Promise.all([getActiveBookmarks(), getTaxonomy()])
      .then(([bookmarks, taxonomy]) => setState({ bookmarks, taxonomy }))
      .catch((err: unknown) => {
        console.error('[home] 读取收藏库失败', err);
        setState({ bookmarks: [], taxonomy: createDefaultTaxonomy() });
      });
  }, []);
          ...
          {expandAll ? (
            <div className="cards">
              {visible.length === 0 ? (
                ...
                visible.map((bookmark) => <BookmarkCard key={bookmark.id} bookmark={bookmark} />)
              )}
            </div>
          ) : (
            <>
